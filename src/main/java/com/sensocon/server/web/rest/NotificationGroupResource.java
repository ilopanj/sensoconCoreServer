...
public class NotificationGroupResource {
    ...
    @Timed
    public List<NotificationGroupDTO> getAllNotificationGroups(@RequestParam(required = false) String filter) {
        if ("sensordevice-is-null".equals(filter)) {
            log.debug("REST request to get all NotificationGroups where sensorDevice is null");
            return notificationGroupService.findAllWhereSensorDeviceIsNull();
        }
